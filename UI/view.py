...
class View(ft.UserControl):
    ...
    def load_interface(self):
        ...
        self.dd_anno = ft.Dropdown(label = "anno", width = 200, on_change= self._controller.leggi_anno,
                                   options=[ft.dropdown.Option(key="None",text="Nessun filtro")])
        self._controller.popola_dd_anno()

        self.dd_brand = ft.Dropdown(label="brand", width=200, on_change=self._controller.leggi_brand,
                                   options=[ft.dropdown.Option(key="None",text="Nessun filtro")])
        self._controller.popola_dd_brand()

        self.dd_retailer = ft.Dropdown(label="retailer", width=500, on_change=self._controller.leggi_retailer,
                                    options=[ft.dropdown.Option(key="None",text="Nessun filtro")])
        ...
        self.btn_analizza_vendite = ft.ElevatedButton(text = "Analizza vendite", on_click=self._controller.get_analizza_vendite)
